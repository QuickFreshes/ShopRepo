...
export default function ShopUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setSmall_address(cleanValues.small_address);
    setLocation(cleanValues.location);
    setLat(cleanValues.lat);
    setLng(cleanValues.lng);
    setShopType(cleanValues.ShopType);
    setAdminSub(cleanValues.AdminSub);
    setErrors({});
  };
  const [shopRecord, setShopRecord] = React.useState(shop);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Shop, idProp) : shop;
